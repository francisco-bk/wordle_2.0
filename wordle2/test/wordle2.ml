...
(*****************************************************************)
(*
[Main.ml]: Manually tested through the terminal. Since [Main] handles the
  interface between the game and the user, it is intuitive to test this by
  playtesting the game using [make play] and checking that the feedback
  is as expected.

[Leaderboard.ml]: Manually tested through the terminal. The leaderboard is
  printed after a game is complete in [make play]. Thus, we are able to verify
  its correctness by playtesting the game and checking that the leaderboard
  updates correctly.

[Load.ml]: Manually tested through the terminal. [Load.ml] provides the
  dictionary that the word to be guessed is pulled from during a game session.
  We are able to verify correctness by playing through the game and ensuring
  that the gameboard selects an answer with the expected number of
  letters.

[RandPick.ml]: Manually tested through the terminal. [RandPick.ml] chooses a
  specific word to be guessed by the player during the game session from a
  dictionary. We are able to verify correctness by playing through the game
  and ensuring that the gameboard selects different words in different sessions.

[HintEngine.ml]: Automatically tested by OUnit. The test cases were developed
  via glass box testing. The hints are generated based off of a keyboard stored
  in [HintEngine.t]. We are able to verify correctness this way because if we
  can test that each of the four keyboards (unguessed, grey, yellow, and green)
...
(** [test_exc name exc input] makes a test named [name] to
verify that [input] raises exception [exc].
Requires: [input] is inside an anonymous function *)
...
[
  ...
  test "parse [' a '] is ['a']" (parse_dict) [" a "] ["a"] (pp_list i) ( = );
  test "parse [' a'] is ['a']" (parse_dict) [" a"] ["a"] (pp_list i) ( = );
  test "parse ['a';'b'] is ['a';'b']" (parse_dict)
  ["a";"b"] ["a";"b"] (pp_list i) ( = );
  test "parse ['a';' b '] is ['a';'b']" (parse_dict)
  ["a";" b "] ["a";"b"] (pp_list i) ( = );
]
...
(* remove y, e, s *)
let alphabet_3rm_tr1 = List.filter
  (fun l -> l <> "y" && l <> "e" && l <> "s") alphabet
...
(* remove z, x, c, v *)
let alphabet_4rm_tr2 = List.filter
  (fun l -> l <> "z" && l <> "x" && l <> "c" && l <> "v") alphabet

let bee_engine_st2 = match get_hint 0 bee_engine_st1 with (_, e) -> e

let bee_engine_st3 = add_guess bee_engine_st2 "eye"

let bee_engine_st4 = match get_hint 1 bee_engine_st3 with (_, e) -> e

let bee_engine_st5 = match get_hint 1 bee_engine_st4 with (_, e) -> e
let hint_bee_engine_st5 = match get_hint 1 bee_engine_st4 with (h, _) -> h
...
[
  ...
  test2 "initialized engine with answer \"yes\" has an unguessed keyboard of all
  26 letters"
  ...
  (* Test-Run 1 state 1 - get three yellow hints from new game with answer
    "yes" *)
  ...
  (yes_engine_st1 |> unguessed_letters) alphabet_3rm_tr1 (pp_list i) cmp_list;
  test2 "taking 3 yellow hints from engine with answer \"yes\" should have
  empty grey keyboard "
  ...
  \"e\", \"s\", and \"a\""
  (yes_engine_st2 |> unguessed_letters) alphabet_4rm_tr1 (pp_list i) cmp_list;
  test2 "guessing \"yea\" after taking 3 yellow hints from engine with answer
  ...
  test2 "initial engine with answer \"bee\" contains the correct answer"
  (bee_engine_st0 |> correct_word) "bee" (fun x -> x) ( = );
  test2 "initial engine with answer \"bee\" has an unguessed keyboard of all
  26 letters"
  (bee_engine_st0 |> unguessed_letters) alphabet (pp_list i) ( cmp_list );
  test2 "initial engine with answer \"bee\" has a grey keyboard of 0 letters"
  (bee_engine_st0 |> grey_letters) [] (pp_list i) ( cmp_list );
  test2 "initial engine with answer \"bee\" has a yellow keyboard of 0 letters"
  (bee_engine_st0 |> yellow_letters) [] (pp_list i) ( cmp_list );
  test2 "initial engine with answer \"bee\" has a green keyboard of 0 letters"
  (bee_engine_st0 |> green_letters) [] (pp_list i) ( cmp_list );

  (* Test-Run 2 state 1 - guess "zxcv" *)
  test2 "guessing \"zxcv\" in engine with answer \"bee\" should have
  unguessed keyboard containing all letters but \"z\", \"x\", \"c\" and \"v\""
  (bee_engine_st1 |> unguessed_letters) alphabet_4rm_tr2 (pp_list i) cmp_list;
  test2 "guessing \"zxcv\" in engine with answer \"bee\" should have
  grey keyboard containing \"z\", \"x\", \"c\" and \"v\""
  (bee_engine_st1 |> grey_letters) ["z"; "x"; "c"; "v"] (pp_list i) cmp_list;
  test2 "guessing \"zxcv\" in engine with answer \"bee\" should have
  empty yellow keyboard"
  (bee_engine_st1 |> yellow_letters) [] (pp_list i) ( cmp_list );
  test2 "guessing \"zxcv\" in engine with answer \"bee\" should have
  empty green keyboard"
  (bee_engine_st1 |> green_letters) [] (pp_list i) ( cmp_list );

  (* Test-Run 2 state 2 - get grey hint *)
  test2 "getting grey hint after guessing four grey letters in engine with
  answer \"bee\" should have unguessed keyboard of size 21"
  (bee_engine_st2 |> unguessed_letters |> List.length) 21 string_of_int (=);
  test2 "getting grey hint after guessing \"zxcv\" in engine with answer
  \"bee\" should have grey keyboard of size 5"
  (bee_engine_st2 |> grey_letters |> List.length) 5 string_of_int (=);
  test2 "After getting grey hint after guessing four grey letters in engine with
  answer \"bee\", combining the grey and unguessed keyboard should result in the
  full alphabet."
  ((bee_engine_st2 |> grey_letters) @ (bee_engine_st2 |> unguessed_letters))
  alphabet (pp_list i) cmp_list;
  test2 "getting grey hint after getting four grey letters in engine with
  answer \"bee\" should have empty yellow keyboard"
  (bee_engine_st2 |> yellow_letters) [] (pp_list i) ( cmp_list );
  test2 "getting grey hint after getting four grey letters in engine
  with answer \"bee\" should have empty green keyboard"
  (bee_engine_st2 |> green_letters) [] (pp_list i) ( cmp_list );

  (* Test-Run 2 state 3 - guess "eye" *)
  test2 "guessing \"eye\" after getting grey hint and guessing four grey letters
  in engine with answer \"bee\" should have unguessed keyboard of size <=21"
  (bee_engine_st3 |> unguessed_letters |> List.length) 21 string_of_int (>=);
  test2 "guessing \"eye\" after getting grey hint and guessing four grey letters
  in engine with answer \"bee\" should have grey keyboard of size >= 5"
  (bee_engine_st3 |> grey_letters |> List.length) 5 string_of_int (<=);
  test2 "guessing \"eye\" after getting grey hint and guessing four grey letters
  in engine with answer \"bee\" should have empty yellow keyboard"
  (bee_engine_st3 |> yellow_letters) [] (pp_list i) ( cmp_list );
  test2 "guessing \"eye\" after getting grey hint and guessing four grey letters
  in engine with answer \"bee\" should have empty green keyboard"
  (bee_engine_st3 |> green_letters) ["e"] (pp_list i) ( cmp_list );

  (* Test-Run 2 state 4 - get one yellow hint *)
  test2 "getting one yellow hint after guessing \"eye\", getting grey hint, and
  guessing four grey letters in engine with answer \"bee\" should have unguessed
  keyboard 1 smaller than in state 3"
  (bee_engine_st4 |> unguessed_letters |> List.length)
  ((bee_engine_st3 |> unguessed_letters |> List.length) - 1) string_of_int (=);
  test2 "getting one yellow hint after guessing \"eye\", getting grey hint, and
  guessing four grey letters in engine with answer \"bee\" should have same grey
  keyboard as in state 3"
  (bee_engine_st4 |> grey_letters) (bee_engine_st3 |> grey_letters)
  (pp_list i) (cmp_list);
  test2 "getting one yellow hint after guessing \"eye\", getting grey hint, and
  ...
  test2 "getting a yellow hint after getting a yellow hint, guessing \"eye\",
  getting grey hint and getting four grey letters in engine with answer \"bee\"
  should have the same unguessed keyboard as state 4"
  (bee_engine_st5 |> unguessed_letters) (bee_engine_st4 |> unguessed_letters)
  (pp_list i) ( cmp_list );
  test2 "getting a yellow hint after getting a yellow hint, guessing \"eye\",
  getting grey hint and getting four grey letters in engine with answer \"bee\"
  should have the same grey keyboard as state 4"
  (bee_engine_st5 |> grey_letters) (bee_engine_st4 |> grey_letters)
  (pp_list i) ( cmp_list );
  test2 "getting a yellow hint after getting a yellow hint, guessing \"eye\",
  getting grey hint and getting four grey letters in engine with answer \"bee\"
  should have the same yellow keyboard as state 4"
  (bee_engine_st5 |> yellow_letters) (bee_engine_st4 |> yellow_letters)
  (pp_list i) ( cmp_list );
  test2 "getting a yellow hint after getting a yellow hint, guessing \"eye\",
  getting grey hint and getting four grey letters in engine with answer \"bee\"
  should have the same green keyboard as state 4"
  (bee_engine_st5 |> green_letters) (bee_engine_st4 |> green_letters)
  (pp_list i) ( cmp_list );
  test2 "getting a yellow hint after getting a yellow hint, guessing \"eye\",
  getting grey hint and getting four grey letters in engine with answer \"bee\"
  should return an invalid hint"
