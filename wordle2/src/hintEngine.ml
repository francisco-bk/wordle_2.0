...
(** Representation of a guessed word. [word] is the guessed word, while
    [colorization] represents the color of each letter in [word]. RI:
    The colorization of the word is a list of 0's, 1's, or 2's of the
    same length as [word]. *)
...
(** Representation of a hint. The [id] is [0] if it is a hint for a grey
    word, and [1] if it is a hint for a yellow word. [letter] is the
    letter given in the hint. *)
...
(** Representation of the hint engine. [answer] is the correct word for
    the current wordle game. [guessed_words] are the words that the
    player has guessed so far. [keyboard] keeps a record of what letters
    the player has used. [hints] records which hints are given to the
    player. *)

(** [init_keyboard ()] initializes the keyboard with all unguessed
    letters. *)
let init_keyboard () =
  {
    unguessed_letters =
...
(** [remove_from_unguessed kb l] is [kb] with letter [l] removed from
    the unguessed letters list. *)
...
let update_keyboard_letter keyboard letter value =
  ...
  | 0 ->
      if List.exists (fun l -> l = letter) greys then keyboard
        ...
        let new_grey = letter :: greys in
        { kb_unguessed with grey_letters = new_grey }
  | 1 ->
      if List.exists (fun l -> l = letter) (greens @ yellows) then
        keyboard
      else
        let new_yellow = letter :: yellows in
        { kb_unguessed with yellow_letters = new_yellow }
  | 2 ->
      if List.exists (fun l -> l = letter) greens then keyboard
      else
        let new_yellow = List.filter (fun l -> l <> letter) yellows in
        {
          kb_unguessed with
          yellow_letters = new_yellow;
          green_letters = letter :: greens;
        }
  | _ -> keyboard

(** [update_keyboard kb color_wd] is [kb] with new colored letters based
    on [color_wd]'s colorings. Precondition: [color_wd] is in the format
    of the output of Processor.colorize_guess. *)
...
(** [update_poss_hints engine hint] is the hint engine with the keyboard
    updated with [hint]'s information. *)
let update_poss_hints engine hint =
  ...
  {
    engine with
    hints = hint :: engine.hints;
    keyboard = updated_keyboard;
  }

(** [unguessed_yellows engine] is the list of yellow unguessed letters
    in state [engine]. *)
...
(** [unguessed_greys engine] is the list of grey unguessed letters in
    state [engine]. *)
let unguessed_greys engine =
  let ungssd = engine.keyboard.unguessed_letters in
  List.filter
    (fun l -> not (String.contains engine.answer l.[0]))
    ungssd
...
let get_hint id engine =
  try
    ...
    | _ -> (None, engine)
  with
  | Invalid_argument _ -> (None, engine)
