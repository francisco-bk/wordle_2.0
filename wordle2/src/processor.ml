...
(** [s_to_list s] takes string [str] and returns a string list with all the 
...
(** [find l e a] searches through list [l] for element [e] starting at index
    [acc] and returns the index of [e]
    Precondition: [e] is an element in [l] *)
let rec find lst element acc : int = 
  let nth_ele = List.nth lst acc in
  if nth_ele = element then acc 
  else find lst element (acc + 1)

(** [replace l i r] replaces the element at index [i] in list [l] with [r], if
    index [i] is not in range of the list [l] then [l] is returned *)
let replace lst index rep = 
  List.mapi (fun i x -> if i = index then rep else x) lst

(** [remove_greens g gr r] takes guess [g] and removes all green indicies [gr] 
    and replaces them with string [r]. *)
let rec remove_greens guess greens rep: string list = 
  match greens with
  | [] -> guess
  | h :: t -> remove_greens (replace guess h rep) t rep

(** [green_list a g acc] returns list of indicies that indicate green coloring 
    based on answer [a] and guess [g] starting at index [acc]. *)
...
(** [yellow_list a g gr acc] returns list of indicies that indicate yellow 
    coloring based on answer [a], guess [g], and green indicies [gr] starting 
    at index [acc]. *)
let rec yellow_list answer guess greens acc : int list = 
  let guess = remove_greens guess greens "#" in
  match answer with 
  | h :: t -> if not (List.mem acc greens) && (List.mem h guess) 
...
(** [combine gr y g a] combines list of green indicies [gr], list of yellow 
    indicies [y], based on guess [g] starting at index [acc]. Returns list of 
    ints representing coloring of each index of guess [g], 
    if green 2, if yellow 1, and if grey 0. *)
